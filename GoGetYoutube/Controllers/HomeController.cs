...
namespace GoGetYoutube.Controllers
{
    ...
    public class HomeController : ControllerBase
    {
        ...
        [HttpPost]
        public IActionResult GetVideo(string YoutubeUrl, bool DownloadSubtitle)
        {
            _logger.LogInformation("Downloading from " + YoutubeUrl);
            if (!Uri.IsWellFormedUriString(YoutubeUrl, UriKind.Absolute))
                return BadRequest("Invalid URL");

            var saveLocationConfig = _appSettings.SaveLocationConfig;
            var qualityConfig = _appSettings.QualityConfig;
            var subtitleConfig = _appSettings.SubTitleConfig;
            string youtubedlConfig = string.Join(" ", new[] { saveLocationConfig, qualityConfig });
            if (DownloadSubtitle)
            {
                youtubedlConfig = string.Join(" ", new[] { subtitleConfig, youtubedlConfig });
            }
